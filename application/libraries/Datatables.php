...
class Datatables {
	...
	/**
	 * Establece columnas para utilizar búsqueda por having
	 *
	 * @param string $columns
	 * @return mixed
	 */
	public function having_column($columns) {
		foreach ($this->explode(',', $columns) as $val) {
			$column = trim(preg_replace('/(.*)\s+as\s+(\w*)/i', '$2', $val));
			$column = preg_replace('/.*\.(.*)/i', '$1', $column);
			$this->having_column[$column] = trim(preg_replace('/(.*)\s+as\s+(\w*)/i', '$1', $val));
		}
		return $this;
	}
	...
	public function generate($output = 'json', $charset = 'UTF-8') {
		...
		if (empty($_POST['columns'])) {
			lm('empty POST en Datatables/generate');
			lm($_POST);
//			return FALSE;
		}
	...
	private function get_filtering() {
		...
		for ($i = 0; $i < count($mColArray); $i++) {
			if (isset($mColArray[$i]['searchable']) && isset($mColArray[$i]['search']['value'])) {
				if ($mColArray[$i]['searchable'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns) && $mColArray[$i]['search']['value'] != '') {
					...
					} elseif (array_key_exists($mColArray[$i]['data'], $this->having_column)) {
						$sSearch = $this->ci->db->escape_like_str(trim($mColArray[$i]['search']['value']));
						$sHaving_and[] = $this->select[$mColArray[$i]['data']] . " LIKE '%" . $sSearch . "%'";
					} else {
						$sSearch = $this->ci->db->escape_like_str(trim($mColArray[$i]['search']['value']));
		...
		if (!empty($sHaving_and)) {
			$sHaving = join(' AND ', $sHaving_and);
		}
